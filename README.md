...
Access at `http://localhost:3000` (default login: `admin` / `tacklebox`). Panels cover active sessions, sessions per hour, average duration, tool usage (totals and over time), tool failures, sessions by directory, file lock warnings, stop blocks, notification types, and subagent activity.

## Testing

```bash
# Create test database (Docker setup)
docker compose exec db psql -U tacklebox -d postgres -c "CREATE DATABASE tacklebox_test OWNER tacklebox;"

# Or for a host-installed Postgres
...
Designed for **localhost use**. The shipped `.claude/settings.json` assumes a trusted local server. For network deployment:

1. Set `API_KEY` to a strong random value, and add `X-API-Key: <value>` to every HTTP hook entry's `headers` and to the `-H` flag of every `curl` hook entry in `.claude/settings.json`. The provided file does not include this header, so enabling `API_KEY` without editing settings will cause all hooks to 401 — failures are silently swallowed by the fail-open layer.
...
5. The `X-API-Key` comparison in `main.py` is non-constant-time; behind TLS this is acceptable, but if you ever expose the server on an untrusted network use `hmac.compare_digest`.

## User Guide

Day-to-day usage — setting project context, managing tasks, querying sessions, debugging context injection, and pre-baked prompts — is documented in [USER_GUIDE.md](USER_GUIDE.md).
